...
export function getType(buzzwords: Buzzwords): string {
    ...
    for (let buzzword in buzzwords.buzzwords ) {
        ...
        for (let i=0; i < buzzWordCount; i++) {
            buzzTypeList.push(mappedBuzzword(buzzword));
        }
    }
    let random: number = Math.floor(Math.random() * buzzTypeList.length);
    return buzzTypeList[random];

}

function mappedBuzzword(buzzword: string) {
    for (let type in typeMap) {
        for (let buzz of typeMap[type]) {
            if (buzz === buzzword) {
                return type;
            }
        }
